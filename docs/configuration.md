...
| media_order                      | The order in which the images are displayed (possible values are: sorted / random / random_but_synced / random_path). With `random_but_synced` every device shows the same pseudo-random image at the same time. With `random_path`, WallPanel randomly selects a media-source folder and displays its media sequentially before selecting another folder. See [Media Sources](media-sources.md#random-path-order). | random     |
