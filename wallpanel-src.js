...
function initWallpanel() {
	...
	class WallpanelView extends HuiView {
			...
			for (let offset = 1; offset < list.length; offset++) {
				const index =
					this.mediaListDirection === "backwards"
						? (this.mediaIndex - offset + list.length) % list.length
						: (this.mediaIndex + offset) % list.length;
				const candidate = list[index];
				if (
					candidate &&
					candidate !== primary &&
					candidate.startsWith("media-source://") &&
		...
		async chooseRandomPortraitPair(img, token) {
			...
			let preferred = candidates.filter(
				(source) =>
					...
					source !== this.lastPortraitPairPrimary &&
					source !== this.lastPortraitPairPartner
			);
			...
			for (const source of [
				...this.shufflePortraitPairCandidates(preferred),
				...this.shufflePortraitPairCandidates(fallback)
			]) {
				if (token !== this.portraitPairToken) return false;
		...
		async preparePortraitPair(img, token) {
			if (
				!config.portrait_pairing ||
				mediaSourceType() !== "media-source" ||
				!img?.infoCacheUrl?.startsWith("media-source://") ||
				this.portraitPairingOrientation(img) !== "portrait"
			)
				return false;
			let paired = false;
